...
pub fn run() {
    ...
    tauri::Builder::default()
        .setup(|app| {
            ...
            // Auto-launch embedded Python FastAPI sidecar
            ...
            if let Some(py_path) = py_sidecar_path {
                if py_path.exists() {
                    ...
                    let child = Command::new(&py_path).spawn().ok();
            ...
            // Auto-launch embedded Java Spring Boot sidecar
            ...
            if let Some(j_path) = java_sidecar_path {
                if j_path.exists() {
                    ...
                    let child = Command::new(&j_path).spawn().ok();
                    if let Ok(mut state) = sidecars.lock() {
